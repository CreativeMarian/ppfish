...
class WebSocketServiceClient:
    ...
    async def recharge_submit(self, order_no: str, phone: str) -> dict:
        """蜜蜂直充-充值页提交：买家在网页输入手机号后触发蜜蜂放单。

        websocket 侧完成 校验订单/手机号/幂等 → 蜜蜂 upload_order → 后台查单轮询 → 结果通知买家，
        返回 {success, message, data:{status, platform_order_no?}}。
        """
        url = f"{self.base_url}/internal/orders/recharge-submit"
        try:
            return await self.http_client.post(url, json={"order_no": order_no, "phone": phone})
        except Exception as e:
            logger.error(f"蜜蜂直充提交失败: {order_no}, 错误: {e}")
            return {"success": False, "message": f"蜜蜂直充提交失败: {str(e)}"}
