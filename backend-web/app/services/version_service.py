...
说明：
- 更新服务器地址与格式跟桌面启动器（launcher/updater.py）保持一致，详见 data/update_config.json。
- 本地版本默认不指向任何作者服务器（_DEFAULT_UPDATE_URL 置空），在线更新默认禁用。
...
# 默认更新服务器地址（本地版本置空，不指向任何作者服务器；需使用时配置 data/update_config.json）
_DEFAULT_UPDATE_URL = ""
