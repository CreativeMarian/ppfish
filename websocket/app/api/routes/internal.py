...
class RechargeSubmitRequest(BaseModel):
    """蜜蜂直充-充值页提交请求（买家在网页输入手机号后由 backend-web 调用）"""
    order_no: str
    phone: str
...
@router.post("/orders/recharge-submit")
async def recharge_submit(request: RechargeSubmitRequest):
    """蜜蜂直充-充值页提交：买家在网页输入手机号后触发蜜蜂放单。

    流程：定位在线账号实例 → 本地订单锁串行 → mf_api_handler.handle_recharge_submit
    （校验订单/手机号/幂等 → 蜜蜂 upload_order → 后台查单轮询 → 结果通知买家）。
    跨进程并发由 backend-web 的 Redis 锁保证。
    """
    from app.services.xianyu.cookie_manager import get_manager
    from common.db.compat import db_manager

    order_no = (request.order_no or "").strip()
    ...
    if not xianyu_live:
        return {
            "success": False, "code": 200,
            "message": "卖家账号当前不在线，暂时无法充值，请稍后重试或联系卖家",
            "data": None,
        }

    local_order_lock = xianyu_live._order_locks[order_no]
    try:
        await asyncio.wait_for(local_order_lock.acquire(), timeout=5)
    except asyncio.TimeoutError:
        return {"success": False, "code": 409, "message": "订单正在处理中，请稍后再试", "data": None}
    try:
        ...
        return await handler._mf_api_handler.handle_recharge_submit(order_no, request.phone)
    finally:
        if local_order_lock.locked():
            local_order_lock.release()
