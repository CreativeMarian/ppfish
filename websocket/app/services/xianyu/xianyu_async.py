...
class XianyuAsync:
    ...
    def __init__(self, cookies_str: str = None, cookie_id: str = "default", user_id: int = None):
        ...
        # 蜜蜂直充：等待买家回复充值手机号的会话状态 {chat_id: {...}}
        self.mf_account_waiting = {}
        ...
        try:
            ...
            if not hasattr(self, 'message_handler'):
                ...
                async def on_chat_message(parsed_message, ws):
                    ...
                    # 0.1 蜜蜂直充：等待买家回复充值手机号
                    # 若买家在下单后回复了手机号，则立即用该手机号继续蜜蜂放单
                    chat_id_for_mf = parsed_message.get("chat_id", "")
                    myid_for_mf = getattr(self, 'myid', self.cookie_id)
                    waiting = getattr(self, 'mf_account_waiting', None) or {}
                    if chat_id_for_mf in waiting and send_user_id != myid_for_mf:
                        try:
                            pending = waiting.pop(chat_id_for_mf, None)
                            if pending:
                                digits = "".join(ch for ch in str(send_message or "") if ch.isdigit())
                                account = digits if len(digits) >= 7 else str(send_message or "").strip()
                                if len(digits) >= 7:
                                    logger.info(
                                        f"【{self.cookie_id}】收到蜜蜂直充手机号: chat_id={chat_id_for_mf}, "
                                        f"order_id={pending.get('order_id')}, account={account}"
                                    )
                                    if hasattr(self, 'auto_delivery_handler') and self.auto_delivery_handler:
                                        rule = pending.get('rule') or {}
                                        await self.auto_delivery_handler._call_mf_api_with_account(
                                            rule=rule,
                                            account=account,
                                            order_id=pending.get('order_id'),
                                            item_id=pending.get('item_id'),
                                            buyer_id=pending.get('buyer_id'),
                                            chat_id=chat_id_for_mf,
                                            send_user_name=parsed_message.get("send_user_name", ""),
                                        )
                                else:
                                    logger.warning(
                                        f"【{self.cookie_id}】蜜蜂直充手机号无效，重新询问: chat_id={chat_id_for_mf}"
                                    )
                                    # 手机号无效，重新询问
                                    await self.auto_delivery_handler._ask_for_mf_recharge_account(
                                        chat_id_for_mf,
                                        pending.get('buyer_id'),
                                        pending.get('rule') or {},
                                        order_id=pending.get('order_id'),
                                        item_id=pending.get('item_id'),
                                    )
                            ...
                            logger.error(f"【{self.cookie_id}】处理蜜蜂直充手机号回复异常: {mf_e}")
