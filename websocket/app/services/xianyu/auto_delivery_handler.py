...
from app.services.xianyu.mf_api_handler import MfApiHandler
...
class AutoDeliveryHandler:
    ...
    def __init__(self, parent):
        ...
        # 初始化蜜蜂汇云直充处理器（mf_api 卡券类型）
        self._mf_api_handler = MfApiHandler(self)
        # 最近一次发货失败原因（用于记录到数据库）
    ...
    @property
    def mf_account_waiting(self):
        """蜜蜂直充等待买家回复手机号的会话状态 {chat_id: {...}}"""
            ...
            if order_id:
                ...
                if rule['card_type'] == 'api':
                    ...
                    if text_content is None:
                        ...
                        logger.info(self._last_delivery_fail_reason)
                        return None
    ...
    # ==================== 蜜蜂汇云直充（委托给MfApiHandler） ====================

    async def _get_mf_api_card_content(self, rule, order_id=None, item_id=None, buyer_id=None, chat_id=None, send_user_name=None):
        """蜜蜂汇云直充发货主入口 - 委托给MfApiHandler处理"""
        return await self._mf_api_handler.get_mf_api_card_content(
            rule, order_id, item_id, buyer_id, chat_id, send_user_name
        )

    async def _call_mf_api_with_account(self, rule, account, order_id=None, item_id=None, buyer_id=None, chat_id=None, send_user_name=None):
        """使用确认的手机号调蜜蜂放单 - 委托给MfApiHandler处理"""
        return await self._mf_api_handler.call_mf_api_with_account(
            rule, account, order_id, item_id, buyer_id, chat_id, send_user_name
        )

    async def _ask_for_mf_recharge_account(self, chat_id, buyer_id, rule, order_id=None, item_id=None):
        """询问买家充值手机号并设置等待状态 - 委托给MfApiHandler处理"""
        return await self._mf_api_handler.ask_for_recharge_account(
