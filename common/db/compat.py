...
class DBManagerCompat:
    ...
    def update_order_mf_status(
        self,
        order_id: str,
        mf_order_no: str = None,
        mf_status: str = None,
        mf_account: str = None,
        mf_error: str = None,
    ) -> bool:
        """更新订单蜜蜂直充状态（记录到 metadata 字段，不依赖额外表列）

        metadata 结构：
            {"mf": {"order_no": "...", "status": "processing/success/failed/...", "account": "...", "error": "..."}}
        """
        async def _update(session_maker):
            async with session_maker() as session:
                stmt = select(XYOrder).where(XYOrder.order_no == order_id)
                ...
                if not order:
                    return False
                meta = dict(order.metadata_json) if isinstance(order.metadata_json, dict) else {}
                mf = dict(meta.get("mf")) if isinstance(meta.get("mf"), dict) else {}
                if mf_order_no is not None:
                    mf["order_no"] = str(mf_order_no)
                if mf_status is not None:
                    mf["status"] = str(mf_status)
                if mf_account is not None:
                    mf["account"] = str(mf_account)
                if mf_error is not None:
                    mf["error"] = str(mf_error)
                meta["mf"] = mf
