...
# 更新服务器地址（从 data/update_config.json 读取；本地版本默认不指向任何作者服务器，置空即禁用在线更新）
_DEFAULT_UPDATE_URL = ""
...
def _get_update_url() -> str:
    ...
    优先从 data/update_config.json 读取，否则使用默认值（空）。
    Returns:
        更新服务器基础URL（不含尾部斜杠），未配置时返回空字符串
    """
    try:
...
def check_update() -> dict:
    ...
    if not update_url:
        result["error"] = "未配置更新服务器地址（本地版本已禁用在线更新）"
        return result
